...
class My_NeuralNetMLP(object):
    ...
    def fit(self, X_train, y_train, X_valid, y_valid):
        ...
        self.w_h = self.random.normal(loc=0.0, scale=0.1, size=(self.n_hidden, n_output))
...
class NeuralNetMLP(object):
    ...
    def fit(self, X_train, y_train, X_valid, y_valid):
        ...
        for i in range(self.epochs):
            ...
            train_acc = ((np.sum(y_train == y_train_pred)).astype(np.float32) /
                         X_train.shape[0])
            valid_acc = ((np.sum(y_valid == y_valid_pred)).astype(np.float32) /
                         X_valid.shape[0])
